...
class select:
    ...
    def __init__(self):
        ...
        self.is_auto_code = _get_yaml()["is_auto_code"]  # 是否自动打码
        self.auto_code_type = _get_yaml()["auto_code_type"]  # 选择哪个打码平台
        self.is_cdn = _get_yaml()["is_cdn"]  # 是否开启CDN
    ...
    def call_login(self, auth=False):
        ...
        """
        if auth:
            return self.login.auth()  # 认证
        else:
            self.login.go_login()  # 登录

    def cdn_req(self, cdn):  # 检查CDN IP是否可用
        for i in range(len(cdn) - 1):  # 逐个检查
            http = HTTPClient(0)
    ...
    def main(self):
        ...
        self.cdn_certification()  # CDN 的过滤
        l = liftTicketInit(self)
        l.reqLiftTicketInit()  # 请求抢票页面
        self.call_login()  # 登录
        check_user = checkUser(self)  # 实例化 "用户登录状态"检查类
        t = threading.Thread(target=check_user.sendCheckUser)  # 周期进行检查"用户的登录状态"
        t.setDaemon(True)
        t.start()  # 设置成 后台进程
        # 读取 出发站 和 目的地站 的位置
        ...
        s.sendGetPassengerDTOs()  # 获取乘车人信息
        while 1:
            try:
                num += 1
                now = datetime.datetime.now()  # 感谢群里大佬提供整点代码
                configCommon.checkSleepTime(self)   # 晚上到点休眠(12306网站晚上不能访问)
                # order_model  抢票模式： 1：预售 2：捡漏
                if self.order_model is 1:  # 设置 轮训的周期
                    sleep_time_s = 0.5
                    sleep_time_t = 0.6
                    # 测试了一下有微妙级的误差，应该不影响，测试结果：2019-01-02 22:30:00.004555，预售还是会受到前一次刷新的时间影响
                    # ，暂时没想到好的解决方案
                    # TODO： 这里应该提前0.1秒的
