    ...
    try:
        if is_auto_code:
            ...
            if auto_code_type == 2:
                ...
                Result = rc.rk_create(result, 6113)  # 进行验证码识别
                if "Result" in Result:
...
def codexy(Ofset=None, is_raw_input=True):
    """
    Ofset： [1, 8] 就是火车票 验证码的 图片点击位置
    is_raw_input： 是否是 手动输入
    获取验证码
    :return: str
    """
    if is_raw_input:
    ...
    Ofset = Ofset.replace("，", ",")
    select = Ofset.split(',')  # 切分出 选定的图片单元
    post = []
    offsetsX = 0  # 选择的答案的left值,通过浏览器点击8个小图的中点得到的,这样基本没问题
    offsetsY = 0  # 选择的答案的top值
    for ofset in select:  # 生成  几个点击 的 x-y 序列
        if ofset == '1':
            offsetsY = 77
            offsetsX = 40
        elif ofset == '2':
